...
class OpenAIService:
    ...
    def _generate_fallback_analysis(symbol: str, name: str, close_price: float, change_percent: float) -> str:
        ...
        if change_percent >= 0:
            return (
                ...
                "【跟你有關】\n"
                "看著持股上漲，您的情緒今天應該比較安心。不過，新手投資人在上漲時，往往容易產生「想趕快追高加碼」或是「想立刻獲利了結」的衝擊情緒。此時建議先深呼吸，按照當初設定的投資目標執行，不要隨市場情緒隨意做買賣決定。\n\n"
                "【可以留意】\n"
            ...
            return (
                ...
                "【跟你有關】\n"
                "看著庫存損益出現紅字跌幅，心裡感到焦慮或緊張是極為正常的。請對自己說一句：這只是市場的日常波動，並不代表這家公司的長期價值歸零。在恐慌時最忌諱做倉促的停損決定，先關掉看盤 App，給情緒一點沉澱的時間。\n\n"
                "【可以留意】\n"
                "建議留意下方的重要支撐均線，以及即將發布的季報數據。若公司的長期產業競爭力沒有變，短期的非理性修正往往是定期定額分批攤平的好機會。今天就別看盤了，好好放鬆一下吧！"
            )
    ...
    async def fetch_stock_analysis(cls, symbol: str, name: str, close_price: float, change_percent: float) -> str:
        ...
        prompt = (
            ...
            f"今日收盤價為 {close_price}，漲跌幅為 {change_percent}% 的表現，為股票新手生成一段溫暖、口語化且排版清晰的分析。\n"
            "分析必須包含以下三個段落（使用繁體中文）：\n"
            "1. 【發生什麼】：說明今天個股的大致走勢與可能的市場因素。\n"
            "2. 【跟你有關】：以對帳單與心理角度分析，持有這檔股票的人今天的心情與損益變動該如何看待。\n"
            "3. 【可以留意】：提供接下來新手可以關注的指標（如支撐點、長期營收等），強調不需要急著做買賣交易決策。"
        )
